...
app.post("/api/insert", (req, res) => {
    ...
    db.query("INSERT INTO employees (email, password, name, surname, position, salary, isStaff) VALUES (?, ?, ?, ?, ?, ?, ?)", [email, password, name, surname, position, salary, isStaff], (err, result) => {
        if (err) {
...
app.post("/api/login", (req, res) => {
    ...
    db.query("SELECT * FROM employees WHERE email = ?", [email], (err, result) => {
        if (err) { res.send({ err:err, status : "error", message: "Fail login" }); }
        if (result.length > 0) {
            bcrypt.compare(password, result[0].password, (err, response) => {
                if (response) {
                    const id = result[0].id;
                    const token = jwt.sign({ id: id }, secret, { expiresIn: '1h' });
                    res.send({ result: result, token: token, status: "ok", message: "Login successfully" });
                } else {
